...
def create_parameters(options):
	...
	# channel id for input or url as input
		...
		if options.cUrl:
		    #eg:https://www.youtube.com/@pradipmohanta9321/videos
			resp = requests.get(options.cUrl)
			soup = BeautifulSoup(resp.text, 'html.parser')
			cid = soup.select_one('meta[property="og:url"]')['content'].strip('/').split('/')[-1]
		else:
			cid = options.chid
...
def main(options):
	if options.user == False and options.chid == False and \
	    options.url == False and options.cUrl == False:
		parser.print_help()
		sys.exit(1)
